...
class Fight extends Command {
    ...
    constructor(client) {
        super(client, {
            ...
            slashCommandOptions: {
                description: "⚔️ Fight a mob in order to gain money"
            }
        });
    };
    ...
    async runInteraction(interaction, data) {
        ...
        // Récupérer le nom de toutes les armes et calculer laquelle est utilisée
        let weapon = interaction.drakeWS("common:HAND");
        let armor = interaction.drakeWS("common:HAND");

        if((data.member.inventory.map((i) => i.name)).includes("wooden_sword")) weapon = interaction.drakeWS("economy/fight:WOODEN_SWORD");
        if((data.member.inventory.map((i) => i.name)).includes("stone_sword")) weapon = interaction.drakeWS("economy/fight:STONE_SWORD");
        if((data.member.inventory.map((i) => i.name)).includes("iron_sword")) weapon = interaction.drakeWS("economy/fight:IRON_SWORD");
        if((data.member.inventory.map((i) => i.name)).includes("diamond_sword")) weapon = interaction.drakeWS("economy/fight:DIAMOND_SWORD");
        if((data.member.inventory.map((i) => i.name)).includes("demonic_sword")) weapon = interaction.drakeWS("economy/fight:DEMONIC_SWORD");
        if((data.member.inventory.map((i) => i.name)).includes("legendary_sword")) weapon = interaction.drakeWS("economy/fight:LEGENDARY_SWORD");

        if((data.member.inventory.map((i) => i.name)).includes("wooden_armor")) armor = interaction.drakeWS("economy/fight:WOODEN_ARMOR");
        if((data.member.inventory.map((i) => i.name)).includes("stone_armor")) armor = interaction.drakeWS("economy/fight:STONE_ARMOR");
        if((data.member.inventory.map((i) => i.name)).includes("iron_armor")) armor = interaction.drakeWS("economy/fight:IRON_ARMOR");
        if((data.member.inventory.map((i) => i.name)).includes("diamond_armor")) armor = interaction.drakeWS("economy/fight:DIAMOND_ARMOR");

        // Calculer les domages et points de vies du joueur
        let damage = weapon !== interaction.drakeWS("common:HAND") ? (data.member.inventory != "" ? (data.member.inventory.find((i) => (lang.moment === "fr" ? i.namefr : i.nameen) === weapon)).damage : 5) : 5;
        let lp = (armor !== interaction.drakeWS("common:HAND") ? (data.member.inventory != "" ? (((data.member.inventory.find((i) => (lang.moment === "fr" ? i.namefr : i.nameen) === armor)).resistance) ? (data.member.inventory.find((i) => (lang.moment === "fr" ? i.namefr : i.nameen) === armor)).resistance : 5) : 5) : 5) + 60;

        // Envoyer le message annoncant l'ennemi & ses pv
        log = interaction.drakeWS("economy/fight:MOB", {
            weapon
        });

        const waitMsg = interaction.drakeWS("misc:PLEASE_WAIT", {
            emoji: "waiting"
        });
        ...
        sendMessage("\n" + interaction.drakeWS("economy/fight:MOB_HP", { mobHP }));
        ...
        async function checkWin() {
            ...
            if(lp > 0) { // Victoire du joueur :]
                ...
                // Envoyer un message de victoire
                sendMessage(interaction.drakeWS("economy/fight:MOB_DEAD", {
                    symbol: data.guild.symbol,
                    amount: toWin.toString()
                }));
                ...
                // Envoyer un message annoncant la mort du joueur
                sendMessage(interaction.drakeWS("economy/fight:DEAD"));
        ...
        async function mobHitPlayer() {
            ...
            // Informer le joueur de l'attaque du mob
            sendMessage(interaction.drakeWS("economy/fight:MOB_DAMAGES", {
                damage: damageOfTheMob
            }));

            // Donner le nombre de points de vie restants du joueur
            sendMessage(interaction.drakeWS("economy/fight:YOUR_HP", {
                lp: lp > 0 ? lp : 0
            }));
        ...
        async function playerHitMob() {
            ...
            // Envoyer le message donnant l'attaque du joueur
            sendMessage(interaction.drakeWS("economy/fight:MOB_DAMAGE", {
                damage
            }));
            ...
            /// Donner le nombre de points de vie du mob
            sendMessage(interaction.drakeWS("economy/fight:DAMAGES", {
                mobHP: mobHP > 0 ? mobHP : 0
            }));
        };
