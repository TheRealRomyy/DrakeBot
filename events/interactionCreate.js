        ...
        if (interaction.isCommand()) {
            ...
            interaction.__proto__.time = {
                ...
                const d = absoluteDays
                    ? absoluteDays === 1
                        ? interaction.drakeWS("time:ONE_DAY")
                        : interaction.drakeWS("time:DAYS", { amount: absoluteDays })
                    : null;
                const h = absoluteHours
                    ? absoluteHours === 1
                        ? interaction.drakeWS("time:ONE_HOUR")
                        : interaction.drakeWS("time:HOURS", { amount: absoluteHours })
                    : null;
                const m = absoluteMinutes
                    ? absoluteMinutes === 1
                        ? interaction.drakeWS("time:ONE_MINUTE")
                        : interaction.drakeWS("time:MINUTES", { amount: absoluteMinutes })
                    : null;
                const s = absoluteSeconds
                    ? absoluteSeconds === 1
                        ? interaction.drakeWS("time:ONE_SECOND")
                        : interaction.drakeWS("time:SECONDS", { amount: absoluteSeconds })
                    : null;
                const ams = ms
                ? ms === 1
                    ? interaction.drakeWS("time:ONE_MILISECOND")
                    : interaction.drakeWS("time:MILISECONDS", { amount: ms })
                : null;
